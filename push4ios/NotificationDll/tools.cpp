...
string GetIPbyHostName(const char* hostname)
{
	...
	HOSTENT *host_entry = gethostbyname(hostname);//First-chance exception 出现两次
	string retstr("");
	if(host_entry!=0)
	{
		char buf[10];
		memset(buf,0,sizeof(buf));
		itoa((int)(host_entry->h_addr_list[0][0]&0x00ff),buf,10);
		retstr.append(buf);
		retstr.append(".");
		
		memset(buf,0,sizeof(buf));
		itoa((int)(host_entry->h_addr_list[0][1]&0x00ff),buf,10);
		retstr.append(buf);
		retstr.append(".");
		memset(buf,0,sizeof(buf));
		itoa((int)(host_entry->h_addr_list[0][2]&0x00ff),buf,10);
		retstr.append(buf);
		retstr.append(".");
		memset(buf,0,sizeof(buf));
		itoa((int)(host_entry->h_addr_list[0][3]&0x00ff),buf,10);
		retstr.append(buf);
	}
	return retstr;
}
...
//////'a'=>'0''A'
char *binarychartoHexchar(const char*src,char*des,int len)
{
	char *retdes = des;
...
//双HEX字符转unsigned char  ;'0''A'=>'A'
//len is src's length
char* Hexchartobinarychar(const char*src,char*des,int len)
{
	unsigned char *desret = (unsigned char *)des;
